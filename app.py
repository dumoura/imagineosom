...
st.title("Imagine o som")

st.markdown("Este app converte uma imagem numa canção. Brinque com as opções abaixo.")
...
st.sidebar.markdown("Seleccione 'Amostra' se pretender utilizar uma das imagens pré-carregadas. Seleccione 'Imagem do Usuário' se pretender utilizar a sua própria imagem.")
_radio = st.sidebar.radio("",("Amostra", "Imagem do Usuário"))

sample_images = glob.glob('*.jpg')
samp_imgs_df = pd.DataFrame(sample_images,columns=['Images'])
samp_img = st.sidebar.selectbox('Escolha uma amostra', samp_imgs_df['Images'])

#Load image 
user_data = st.sidebar.file_uploader(label="Escolha uma imagem do seu computador")
if _radio == "Amostra":
    img2load = samp_img
elif _radio == "Imagem do Usuário": 
    img2load = user_data
...
with col1:
    scale = st.selectbox('Escolha uma escala', df1['Scale_Choice'])

    'Escala: ' + scale + ' scale'
with col2:
    key = st.selectbox('Escolha uma tonalidade', df2['Keys']) 
    
    'Tom: ', key

with col3:
    octave = st.selectbox('Defina uma oitava', df3['Octaves']) 

    'Oitava: ', octave
with col4:
    harmony = st.selectbox('Defina uma harmonia', df4['Harmonies']) 
...
with col5:
    ...
    random_pixels = st.checkbox('Utilizar pixéis aleatórios para construir da música', value=True)
with col6:
    #Ask user to select song duration
    use_octaves = st.checkbox('Randomizar as oitavas durante a construção da música', value=True) 
    
col7, col8 = st.columns(2)
with col7:
    #Ask user to select song duration
    t_value = st.slider('Duração da nota [s]', min_value=0.01, max_value=1.0, step = 0.01, value=0.2)     

with col8:
    #Ask user to select song duration
    n_pixels = st.slider('Quantos píxeis utilizar? (Mais pixéis demoram mais tempo)', min_value=12, max_value=320, step=1, value=60)         
...
with ps1:
    ...
    semitones   = st.slider('semitons', min_value=0.0, max_value=12.0, step=1.0, value=0.0) 
